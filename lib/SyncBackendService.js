...
class SyncBackendService {
  ...
  /**
   * Opens DB and returns a new service instance.
   * @param {{ runMigration?: boolean }} options
   ...
   * @returns {Promise<PushSubscriptionJSON&{refreshedAt: string}|null>}
   */
  async findPushSubscription (endpoint) {
    ...
    return {
      ...JSON.parse(subscriptionRow.data),
      refreshedAt: subscriptionRow.updatedAt || subscriptionRow.createdAt
    }
  }
   ...
   * @returns {Promise<number>}
   */
  async refreshPush (endpoint) {
    const refreshedAt = new Date().toISOString()
    const { lastID, changes } = await this.runAsync(`
      UPDATE pushes 
      SET version = version + 1, updatedAt = $refreshedAt
      WHERE endpoint = $endpoint
    `, {
      $endpoint: endpoint,
