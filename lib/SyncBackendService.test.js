...
describe('SyncBackendService', () => {
  ...
  it('removePush', async () => {
    ...
    // unknown endpoint
    await expect(service.removePush('456')).rejects.toThrow('An unknown deleting error')
  })

  it('refreshPush', async () => {
    const pushId = await service.savePush({
      endpoint: '123',
      keys: { auth: 'abc', p256dh: 'xyz' }
    })
    ...
    const subscriptionBefore = await service.findPushSubscription('123')
    expect(subscriptionBefore.refreshedAt).not.toBe('')

    await new Promise(resolve => setTimeout(resolve, 10))
    const refreshedId = await service.refreshPush('123')
    expect(refreshedId).toEqual(pushId)

    const subscriptionAfter = await service.findPushSubscription('123')
    expect(subscriptionAfter.refreshedAt).not.toEqual(subscriptionBefore.refreshedAt)
  })
})
